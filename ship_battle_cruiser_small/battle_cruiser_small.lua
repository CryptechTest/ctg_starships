...
function ship_battle_cruiser_small.register_cruiser(custom_data)

    local data = custom_data or {}

    data.typename = custom_data.typename or "ship_battle_cruiser_small"
    ...
    local on_receive_fields = function(pos, formname, fields, sender)
        ...
        if locked then
            local formspec = ship_battle_cruiser_small.update_formspec(pos, data, loc, false,
                "Interface lockout!\nAllocator is busy...")
            meta:set_string("formspec", formspec)
            return
        end
        ...
        --[[if fields.submit_dock then
            ...
            if #bays > 0 and vector.distance(jpos, spos) <= 768 then
                for c = 1, #bays do
                    ...
                    if bnode.name ~= "ship_battle_cruiser_small:protect2" and bnode.name == "vacuum:vacuum" then
        ...
        local ncount, dest = ship_battle_cruiser_small.get_jump_dest(jpos, offset, data.size)
        ...
        elseif fields.submit_nav and ((is_deepspace and loc ~= "0") or (not is_deepspace)) then
            local formspec = ship_battle_cruiser_small.update_formspec(pos, data, loc, false, "Preparing Jump...")
            meta:set_string("formspec", formspec)
            ...
            local function jump_callback(j)
                ...
                if j == 1 then
                    ...
                    local ready = meta:get_int("travel_ready")
                    local formsp = ship_battle_cruiser_small.update_formspec(pos, data, loc, ready, message)
                    meta:set_string("formspec", formsp)
                    minetest.after(3.5, function()
                        local metad = minetest.get_meta(panel_dest)
                        local formspec_old = ship_battle_cruiser_small.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        meta:set_string("formspec", formspec_old)
                        local formspec_new = ship_battle_cruiser_small.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        metad:set_string("formspec", formspec_new)
                        metad:set_int("travel_ready", 0)
                        minetest.after(5, function()
                            local formspec_rdy = ship_battle_cruiser_small.update_formspec(panel_dest, data, loc, false, "Ready...")
                ...
                local ready = meta:get_int("travel_ready")
                local formspec = ship_battle_cruiser_small.update_formspec(pos, data, loc, ready, message)
                meta:set_string("formspec", formspec)
            end

            -- async jump with callback
            ship_battle_cruiser_small.engine_do_jump(pos, dest, data.size, jump_callback, offset)
    ...
    minetest.register_node(modname .. ":" .. ltier .. "_" .. lmachine_name .. "", {
        ...
        digiline = {
            ...
            effector = {
                action = ship_battle_cruiser_small.scout_digiline_effector
            }
        }
    })
