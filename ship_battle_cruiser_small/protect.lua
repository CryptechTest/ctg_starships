...
ship_battle_cruiser_small.protector = {
    mod = "ship_battle_cruiser_small",
    modpath = MP,
    intllib = S,
...
local protector_flip = minetest.settings:get_bool("ship_battle_cruiser_small.protector_flip") or false
local protector_hurt = tonumber(minetest.settings:get("ship_battle_cruiser_small.protector_hurt")) or 1
local protector_show = tonumber(minetest.settings:get("ship_battle_cruiser_small.protector_show_interval")) or 10
local protector_msg = minetest.settings:get_bool("ship_battle_cruiser_small.protector_msg") ~= false
...
-- protector interface
ship_battle_cruiser_small.protector_formspec = function(meta)
...
ship_battle_cruiser_small.protector.can_dig = function(s, pos, digger, onlyowner, infolevel)
    ...
    local pos = minetest.find_nodes_in_area({
        ...
        y = (pos.y + s.h) + 2,
        z = pos.z + s.l
    }, {"ship_battle_cruiser_small:shield_protect"}) -- "ship_battle_cruiser_small:shield_protect",
    ...
    if pos and #pos > 0 and pos[1] ~= nil then
        ...
        if jumpdrive.name ~= "ship_battle_cruiser_small:jump_drive" then
            -- pos = jumpdrive_pos
        end
    end
...
function minetest.is_protected(pos, digger)
    ...
    if not ship_battle_cruiser_small.protector.can_dig(ship_battle_cruiser_small.protector.size, pos, digger, false, 1) then
        return true
    end
...
local check_overlap = function(itemstack, placer, pointed_thing)
    ...
    local size = {
        l = ship_battle_cruiser_small.protector.size.l * 2,
        w = ship_battle_cruiser_small.protector.size.w * 2,
        h = ship_battle_cruiser_small.protector.size.h * 2
    }

    -- make sure protector doesn't overlap any other player's area
    if not ship_battle_cruiser_small.protector.can_dig(size, pos, name, true, 3) then
...
local del_display = function(pos)
    ...
    for _, v in ipairs(objects) do
        ...
        if v and v:get_luaentity() and v:get_luaentity().name == "ship_battle_cruiser_small:display" then
            v:remove()
        end
    end
end
...
function ship_battle_cruiser_small.rightclick(pos, clicker)
    ...
    if meta and ship_battle_cruiser_small.protector.can_dig(s, pos, name, true, 1) then
        player_pos[name] = pos
        minetest.show_formspec(name, "ship_battle_cruiser_small:node", ship_battle_cruiser_small.protector_formspec(meta))
    end
end

function ship_battle_cruiser_small.punch(pos, node, puncher)
    if minetest.is_protected(pos, puncher:get_player_name()) then
        return
    end
    minetest.add_entity(pos, "ship_battle_cruiser_small:display")
end

-- protection node
minetest.register_node("ship_battle_cruiser_small:shield_protect", {
    ...
    after_place_node = function(pos, placer)
        ...
        meta:set_int("p_width", ship_battle_cruiser_small.protector.size.w)
        meta:set_int("p_length", ship_battle_cruiser_small.protector.size.l)
        meta:set_int("p_height", ship_battle_cruiser_small.protector.size.h)
    end,
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        ship_battle_cruiser_small.protector.can_dig(ship_battle_cruiser_small.protector.size, pointed_thing.under, user:get_player_name(), false, 2)
    end,
    ...
    on_punch = function(pos, node, puncher)
        ...
        minetest.add_entity(pos_offset, "ship_battle_cruiser_small:display")
    end,

    can_dig = function(pos, player)
        -- return player and ship_battle_cruiser_small.protector.can_dig(1, pos, player:get_player_name(), true, 1)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if formname ~= "ship_battle_cruiser_small:node" then
        return
    end
    ...
    if not ship_battle_cruiser_small.protector.can_dig(s, pos, player:get_player_name(), true, 1) then
        return
    end
    ...
    if nod ~= "ship_battle_cruiser_small:protect" and nod ~= "ship_battle_cruiser_small:shield_protect" then
        player_pos[name] = nil
        return
    end
    ...
    minetest.show_formspec(name, formname, ship_battle_cruiser_small.protector_formspec(meta))
end)

-- display entity shown when protector node is punched
minetest.register_entity("ship_battle_cruiser_small:display", {
    physical = false,
    ...
    textures = {"ship_battle_cruiser_small:display_node"},
    timer = 0,
    glow = 10,
...
local x = ship_battle_cruiser_small.protector.size.w
local y = ship_battle_cruiser_small.protector.size.h
local z = ship_battle_cruiser_small.protector.size.l
minetest.register_node("ship_battle_cruiser_small:display_node", {
...
function ship_battle_cruiser_small.do_particles(pos, amount)
    local prt = {
        texture = {
...
minetest.register_abm({
    ...
    nodenames = {"ship_battle_cruiser_small:shield_protect"},
    interval = 1,
    chance = 3,
    min_y = vacuum.vac_heights.space.start_height,
    action = function(pos)

        ship_battle_cruiser_small.do_particles(pos, 7)
