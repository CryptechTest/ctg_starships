    ...
    core.register_entity(def.name, {
        ...
        on_step = function(self, dtime)
            ...
            if self.node_hit then
                ...
                --Hit sound
                if def.hit_sound then
                    core.sound_play(def.hit_sound, {pos=self.previous_pos, pitch=7.51, gain=def.hit_sound_gain, max_hear_distance=2*64})
                end
...
function ship_weapons.launch_energy_rail_projectile(custom_def, operator, origin, target, object_target)
    ...
    local function spawn_proj(_pos)
        ...
        --Fire sound
        if def.fire_sound then core.sound_play(def.fire_sound, {pos=_pos, pitch=math.random(0.85,0.875), gain=def.fire_sound_gain, max_hear_distance=2*28}) end
    end
