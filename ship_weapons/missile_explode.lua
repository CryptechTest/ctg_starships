...
local function missile_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	core.log("action", "MISSILE owned by " .. owner .. " detonated at " ..
...
local function missile_safe_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	if ship_combat_ready then
		...
		if ship_shield > 0 then
			hit_damage = hit_damage + 1
			ship_shield = ship_shield - math.floor(hit_damage * 3 + radius + 0.5)
			if ship_shield <= 0 then
				ship_shield = 0
			end
		...
		if ship_hp > 0 and ship_shield <= 0 then
			hit_damage = hit_damage + 1
			ship_hp = ship_hp - math.floor(hit_damage * 2 + radius + 0.5)
...
function ship_weapons.safe_boom(pos, def)
	...
	if shield > 0 then
		...
		core.sound_play("ctg_shield_hit", {pos = pos, gain = 1.5, pitch = ship_weapons.randFloat(0.25,0.35),
	...
	core.log("action", "A SAFE MISSILE explosion occurred at " .. core.pos_to_string(pos) ..
		" with radius " .. radius)
end
...
function ship_weapons.boom(pos, def)
	...
	core.log("action", "A MISSILE explosion occurred at " .. core.pos_to_string(pos) ..
		" with radius " .. radius)
end
