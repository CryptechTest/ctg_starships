...
function ship_cargo.register_cruiser(custom_data)

    local data = custom_data or {}

    data.min_dist = custom_data.min_dist or 99
    data.typename = custom_data.typename or "ship_cargo"
    ...
    local machine_desc = "Cargo Cruiser"
    ...
    local on_receive_fields = function(pos, formname, fields, sender)
        ...
        if fields.submit_migr then
            ...
            local formspec = ship_cargo.update_formspec(pos, data, 0, ready, message)
            meta:set_string("formspec", formspec)
            return
        end
        ...
        if fields.refresh then
            local ready = meta:get_int("travel_ready")
            local formspec = ship_cargo.update_formspec(pos, data, 0, ready, '')
            meta:set_string("formspec", formspec)
            return
        end

        local jpos = ship_cargo.get_jumpdrive(pos, data.size)
        if jpos == nil then
        ...
        if locked then
            local formspec = ship_cargo.update_formspec(pos, data, loc, false,
                "Interface lockout!\nAllocator is busy...")
            meta:set_string("formspec", formspec)
            return
        end
        ...
        --[[if fields.submit_dock then
            ...
            if #bays > 0 and vector.distance(jpos, spos) <= 768 then
                for c = 1, #bays do
                    ...
                    if bnode.name ~= "ship_cargo:protect2" and bnode.name == "vacuum:vacuum" then
                        ...
                        local formspec = ship_cargo.update_formspec(pos, data, loc, 1, message)
                        meta:set_string("formspec", formspec)
                        break
                    end
                end
            ...
            if not bFound then
                ...
                local formspec = ship_cargo.update_formspec(pos, data, loc, 0, message)
                meta:set_string("formspec", formspec)
                return;
            end
        end]] --
        ...
        local ncount, dest = ship_cargo.get_jump_dest(jpos, offset, data.size)
        ...
        elseif fields.submit_nav and ((is_deepspace and loc ~= "0") or (not is_deepspace)) then
            local formspec = ship_cargo.update_formspec(pos, data, loc, false, "Preparing Jump...")
            meta:set_string("formspec", formspec)
            ...
            local function jump_callback(j)
                ...
                if j == 1 then
                    ...
                    local ready = meta:get_int("travel_ready")
                    local formsp = ship_cargo.update_formspec(pos, data, loc, ready, message)
                    meta:set_string("formspec", formsp)
                    minetest.after(3.5, function()
                        local metad = minetest.get_meta(panel_dest)
                        local formspec_old = ship_cargo.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        meta:set_string("formspec", formspec_old)
                        local formspec_new = ship_cargo.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        metad:set_string("formspec", formspec_new)
                        metad:set_int("travel_ready", 0)
                        minetest.after(5, function()
                            local formspec_rdy = ship_cargo.update_formspec(panel_dest, data, loc, false, "Ready...")
                ...
                local ready = meta:get_int("travel_ready")
                local formspec = ship_cargo.update_formspec(pos, data, loc, ready, message)
                meta:set_string("formspec", formspec)
            end

            -- async jump with callback
            ship_cargo.engine_do_jump(pos, dest, data.size, jump_callback, offset)
    ...
    minetest.register_node(modname .. ":" .. ltier .. "_" .. lmachine_name .. "", {
        ...
        on_punch = function(pos, node, puncher)
            local drive_loc = ship_cargo.get_jumpdrive(pos, data.size)
            if drive_loc then
        ...
        digiline = {
            ...
            effector = {
                action = ship_cargo.scout_digiline_effector
            }
        }
    })
