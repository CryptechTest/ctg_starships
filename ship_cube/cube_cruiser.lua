...
function ship_cube.register_cruiser(custom_data)

    local data = custom_data or {}

    data.min_dist = custom_data.min_dist or 50
    data.typename = custom_data.typename or "ship_cube"
    data.modname = custom_data.modname or "ship_cube"
    data.tier = custom_data.tier or "HV"
    data.machine_name = custom_data.machine_name
    data.machine_desc = custom_data.machine_desc or "Cube Cruiser"
    ...
    local machine_desc = "Cube Cruiser"
    ...
    local on_receive_fields = function(pos, formname, fields, sender)
        ...
        if fields.submit_migr then
            ...
            local formspec = ship_cube.update_formspec(pos, data, 0, ready, message)
            meta:set_string("formspec", formspec)
            return
        end
        ...
        if fields.refresh then
            local ready = meta:get_int("travel_ready")
            local formspec = ship_cube.update_formspec(pos, data, 0, ready, '')
            meta:set_string("formspec", formspec)
            return
        end
        ...
        if locked then
            local formspec =
                ship_cube.update_formspec(pos, data, loc, false, "Interface lockout!\nAllocator is busy...")
            meta:set_string("formspec", formspec)
            return
        end
        ...
        --[[if fields.submit_dock then
            ...
            if #bays > 0 and vector.distance(jpos, spos) <= 768 then
                for c = 1, #bays do
                    ...
                    if bnode.name ~= "ship_cube:protect2" and bnode.name == "vacuum:vacuum" then
        ...
        elseif fields.submit_nav and ((is_deepspace and loc ~= "0") or (not is_deepspace)) then
            local formspec = ship_cube.update_formspec(pos, data, loc, false, "Preparing Jump...")
            meta:set_string("formspec", formspec)
            ...
            local function jump_callback(j)
                ...
                if j == 1 then
                    ...
                    local ready = meta:get_int("travel_ready")
                    local formsp = ship_cube.update_formspec(pos, data, loc, ready, message)
                    meta:set_string("formspec", formsp)
                    minetest.after(3.5, function()
                        local metad = minetest.get_meta(panel_dest)
                        local formspec_old = ship_cube.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        meta:set_string("formspec", formspec_old)
                        local formspec_new = ship_cube.update_formspec(panel_dest, data, loc, false, "Jump Complete!")
                        metad:set_string("formspec", formspec_new)
                        metad:set_int("travel_ready", 0)
                        minetest.after(5, function()
                            local formspec_rdy = ship_cube.update_formspec(panel_dest, data, loc, false, "Ready...")
                ...
                local ready = meta:get_int("travel_ready")
                local formspec = ship_cube.update_formspec(pos, data, loc, ready, message)
                meta:set_string("formspec", formspec)
            end
    ...
    minetest.register_node(modname .. ":" .. ltier .. "_" .. lmachine_name .. "", {
        ...
        on_construct = function(pos)
            ...
            meta:set_int("travel_ready", 0)
            meta:set_string("formspec", ship_cube.update_formspec(pos, data, "0", false, ''))
        ...
        digiline = {
            ...
            effector = {
                action = ship_cube.scout_digiline_effector
            }
        }
    })
