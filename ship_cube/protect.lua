...
local protector_flip = minetest.settings:get_bool("ship_cube.protector_flip") or false
local protector_hurt = tonumber(minetest.settings:get("ship_cube.protector_hurt")) or 1
local protector_show = tonumber(minetest.settings:get("ship_cube.protector_show_interval")) or 7
local protector_msg = minetest.settings:get_bool("ship_cube.protector_msg") ~= false
...
-- protector interface
ship_cube.protector_formspec = function(meta)
...
ship_cube.protector.can_dig = function(s, pos, digger, onlyowner, infolevel)
    ...
    local pos = minetest.find_nodes_in_area({
        ...
        y = (pos.y + s.h) + 2,
        z = pos.z + s.l
    }, {"ship_cube:protect2"}) -- "ship_cube:protect2",
    ...
    if pos and #pos > 0 and pos[1] ~= nil then
        ...
        if jumpdrive.name ~= "ship_cube:jump_drive" then
            --pos = jumpdrive_pos
        end
    end
...
function minetest.is_protected(pos, digger)
    ...
    if not ship_cube.protector.can_dig(ship_cube.protector.size, pos, digger, false, 1) then
        return true
    end
...
local check_overlap = function(itemstack, placer, pointed_thing)
    ...
    local size = {
        l = ship_cube.protector.size.l * 2,
        w = ship_cube.protector.size.w * 2,
        h = ship_cube.protector.size.h * 2
    }

    -- make sure protector doesn't overlap any other player's area
    if not ship_cube.protector.can_dig(size, pos, name, true, 3) then
...
local del_display = function(pos)
    ...
    for _, v in ipairs(objects) do
        ...
        if v and v:get_luaentity() and v:get_luaentity().name == "ship_cube:display" then
            v:remove()
        end
    end
end
...
function ship_cube.rightclick(pos, clicker)
    ...
    if meta and ship_cube.protector.can_dig(s, pos, name, true, 1) then
        player_pos[name] = pos
        minetest.show_formspec(name, "ship_cube:node", ship_cube.protector_formspec(meta))
    end
end

function ship_cube.punch(pos, node, puncher)
    if minetest.is_protected(pos, puncher:get_player_name()) then
        return
    end
    minetest.add_entity(pos, "ship_cube:display")
end

-- protection node
minetest.register_node("ship_cube:protect2", {
    ...
    after_place_node = function(pos, placer)
        ...
        meta:set_int("p_width", ship_cube.protector.size.w)
        meta:set_int("p_length", ship_cube.protector.size.l)
        meta:set_int("p_height", ship_cube.protector.size.h)
    end,
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        ship_cube.protector.can_dig(ship_cube.protector.size, pointed_thing.under,
    ...
    on_punch = function(pos, node, puncher)
        ...
        minetest.add_entity(pos_offset, "ship_cube:display")
    end,

    can_dig = function(pos, player)
        -- return player and ship_cube.protector.can_dig(1, pos, player:get_player_name(), true, 1)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if formname ~= "ship_cube:node" then
        return
    end
    ...
    if not ship_cube.protector.can_dig(s, pos, player:get_player_name(), true, 1) then
        return
    end
    ...
    if nod ~= "ship_cube:protect" and nod ~= "ship_cube:protect2" then
        player_pos[name] = nil
        return
    end
    ...
    minetest.show_formspec(name, formname, ship_cube.protector_formspec(meta))
end)

-- display entity shown when protector node is punched
minetest.register_entity("ship_cube:display", {
    physical = false,
    ...
    textures = {"ship_cube:display_node"},
    timer = 0,
    glow = 10,
...
local x = ship_cube.protector.size.w
local y = ship_cube.protector.size.h
local z = ship_cube.protector.size.l
minetest.register_node("ship_cube:display_node", {
...
function ship_cube.do_particles(pos, amount)
    local prt = {
        texture = {
...
minetest.register_abm({
    ...
    nodenames = {"ship_cube:protect2"},
    interval = 1,
    chance = 3,
    min_y = vacuum.vac_heights.space.start_height,
    action = function(pos)

        ship_cube.do_particles(pos, 7)
