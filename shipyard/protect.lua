...
function minetest.is_protected(pos, digger)
    ...
    if not shipyard.protector.can_dig(shipyard.size, pos, digger, false, 1) then
        return true
    end
...
-- protection node
minetest.register_node("shipyard:protect2", {
    ...
    after_place_node = function(pos, placer)
        ...
        meta:set_int("p_width", shipyard.size.w);
        meta:set_int("p_length", shipyard.size.l);
        meta:set_int("p_height", shipyard.size.h);
    end,
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        shipyard.protector.can_dig(shipyard.size, pointed_thing.under, user:get_player_name(), false, 2)
    end,
    ...
    can_dig = function(pos, player)
        -- return player and shipyard.protector.can_dig(1, pos, player:get_player_name(), true, 1)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if nod ~= "shipyard:protect" and nod ~= "shipyard:protect2" then
        player_pos[name] = nil
        return
    end
...
local x = shipyard.size.w
local y = shipyard.size.h
local z = shipyard.size.l
