...
function ship_scout_legacy.update_formspec(pos, data, loc, ready, message)
    ...
    if typename == 'ship_scout_legacy' then
...
function ship_scout_legacy.get_protector(pos, size)
    ...
    local nodes = minetest.find_nodes_in_area(pos1, pos2, {"ship_scout_legacy:shield_protect", "ship_scout_legacy:shield_protect_legacy"})
