...
function ship_raider.update_formspec(pos, data, loc, ready, message)
...
function ship_raider.engine_do_jump(pos, dest, size, jump_callback, dest_offset)
...
function ship_raider.engine_jump_activate(pos, dest, size)
    local pos1 = vector.subtract(pos, {
        x = size.w,
        y = size.h,
        z = size.l
...
function ship_raider.get_jump_dest(pos, offset, size)
    local pos1 = vector.subtract(pos, {
        x = size.w,
        y = size.h,
        z = size.l
...
function ship_raider.get_jumpdrive(pos, size)
    local pos1 = vector.subtract(pos, {
        x = size.w,
        y = size.h,
        z = size.l
...
function ship_raider.get_protector(pos, size)
    local pos1 = vector.subtract(pos, {
        x = size.w,
        y = size.h,
        z = size.l
    ...
    local nodes = minetest.find_nodes_in_area(pos1, pos2, "ship_raider:shield_protect")
